...
const CoursesPending = () => {
    ...
    const { data, isLoading } = useQuery({
      ...
      queryFn: async () => {
        const { response, error } = await courseApi.getByStatus(CourseStatus.PENDING, { pageNumber: page, pageSize: 5 }, user.id)
                ...
                ?   (
                    <div className='w-full h-80 rounded-xl p-10 flex flex-col items-center justify-center
                        ...
                        <div className='flex flex-col text-gray-500 text-xl'>
                            <span>Rất tiếc. Bạn chưa có khóa học nào đang chờ duyệt xuất bản!</span>
                            <span>Hoàn thành bản nháp và yêu cầu xuất bản khóa học của bạn</span>
                        </div>
                    </div>
                )
                :   (
                                                ...
                                                data?.content.map((course, _) => (
                                                    <tr key={course.id}>
                                                        ...
                                                        <td className='py-3'>
                                                            ...
                                                            <i className="w-2 h-2 rounded-full bg-pending"></i>
                                                                Chờ duyệt xuất bản
                                                            </span>
                                                        </td>
                                                        {/* <td className='py-3'>
